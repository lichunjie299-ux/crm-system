...
const Opportunities = {
  ...
  renderList(stageFilter) {
    ...
    const renderBriefStatsHtml = (stats) => {
      ...
      const cardColors = {
        '整体':      { bg: '#e8f0fe', color: '#374151' },
        '需求待确认': { bg: '#dbeafe', color: '#374151' },
        '需求确认':   { bg: '#dbeafe', color: '#374151' },
        '方案认可':   { bg: '#dbeafe', color: '#374151' },
        '确定合作':   { bg: '#dbeafe', color: '#374151' },
        '合同签约':   { bg: '#dbeafe', color: '#374151' },
        '赢单':      { bg: '#d1fae5', color: '#065f46' },
        '输单':      { bg: '#fee2e2', color: '#991b1b' },
      };
      ...
      const buildCard = (label, count, amount) => {
        ...
        return `<div style="flex:1;min-width:72px;background:${c.bg};border-radius:6px;padding:6px 8px;text-align:center;cursor:help" title="${buildTip(label)}">
          <div style="font-size:12px;color:${c.color};opacity:0.9;margin-bottom:2px;line-height:1.3">${label}</div>
          <div style="font-size:16px;font-weight:700;color:${c.color};line-height:1.2;margin-bottom:1px">${count}</div>
          <div style="font-size:12px;color:${c.color};opacity:0.85;line-height:1.2">${fmtAmt(amount)}</div>
        </div>`;
      };
