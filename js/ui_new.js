    ...
    const html = fields.map(f => {
      ...
      switch (f.type) {
        ...
        case 'textarea':
          ...
          break;
        case 'date':
          input = `<input type="date" class="form-input" name="${f.key}" value="${Helpers.escapeHtml(String(value))}" placeholder="${f.placeholder || ''}" ${f.disabled ? 'disabled' : ''}>`;
          break;
        case 'number':
        ...
        case 'dropConditionBuilder':
          ...
          break;
        case 'section':
          return `<div class="form-section-title">${Helpers.escapeHtml(f.label)}</div>`;
        default:
